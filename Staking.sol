...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    mapping(address => mapping(address => mapping(uint256=>uint256))) public lockedUserRewardPerTokenPaid;
    mapping(address => mapping(address => mapping(uint256=>uint256))) public lockedRewards;
    ...
    function claimableRewards(address account) external view returns (RewardData[] memory rewardsAmount) {
        rewardsAmount = new RewardData[](rewardTokens.length);
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            rewardsAmount[i].token = rewardTokens[i];
            rewardsAmount[i].amount = _earned(account, rewards[i].token, balance, supply);
        }
 ...
 /*    // Information on the "earned" balances of a user
  ...
  /*   // Final balance received and penalty balance paid by user upon calling exit
    ...
    function withdrawableRewards(address user)view public returns(uint256 amount,uint256 penaltyAmount){
        Balances storage bal = balances[user];
        if (bal.locked > 0) {
            uint256 amountWithoutPenalty;
            uint256 length = userEarnings[user].length;
    ...
    function stake(uint256 amount, bool lock) external nonReentrant updateReward(msg.sender) {
        ...
        if (lock) {
            ...
            if (idx == 0 || userLocks[msg.sender][idx-1].unlockTime < unlockTime) {
                userLocks[msg.sender].push(LockedBalance({id: idx,amount: amount, unlockTime: unlockTime}));
            } else {
  ...
  /*   function mint(address user, uint256 amount) external updateReward(user) {
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        if (amount <= bal.unlocked) {
            ...
            for (uint i = 0; ; i++) {
                ...
                if (userLocks[msg.sender][i].unlockTime > block.timestamp) {

                     uint penaltyReward = lockedRewards[msg.sender][rewardTokens[0]][userLocks[msg.sender][i].id];
                     penaltyReward=penaltyReward.div(2);
                     penaltyAmount+=penaltyReward;
                     lockedRewards[msg.sender][rewardTokens[0]][userLocks[msg.sender][i].id]=penaltyReward;
                   /*  penaltyAmount = remaining;
                    //require(bal.locked >= remaining, "Insufficient balance after penalty");
                   // bal.locked = bal.earned.sub(remaining);
                    if (bal.locked == 0) {
        ...
        IERC20(rewardTokens[0]).safeTransfer(msg.sender, penaltyAmount);
        if (penaltyAmount > 0) {
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        for (uint i; i < rewardTokens.length; i++) {
            ...
            uint totalUnlockedRewards;
            for(uint j;j<userLocks[msg.sender].length;j++){
                if(block.timestamp> userLocks[msg.sender][j].unlockTime){
                    uint lockedReward=lockedRewards[msg.sender][_rewardsToken][userLocks[msg.sender][j].id];
                    if(lockedReward>0){
                        lockedRewards[msg.sender][_rewardsToken][userLocks[msg.sender][j].id]=0;
                        totalUnlockedRewards+=lockedReward;
                    }
                }
            }
            totalUnlockedRewards=totalUnlockedRewards+reward;
            IERC20(_rewardsToken).safeTransfer(msg.sender, totalUnlockedRewards);
            emit RewardPaid(msg.sender, _rewardsToken, totalUnlockedRewards);
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            for(uint i=0;i<userLocks[msg.sender].length;i++){
                lockedRewards[account][token][userLocks[msg.sender][i].id] = _earned(account, token, userLocks[msg.sender][i].amount, supply);
                lockedUserRewardPerTokenPaid[account][token][userLocks[msg.sender][i].id]= rewardData[token].rewardPerTokenStored;
